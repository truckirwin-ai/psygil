# CLAUDE.md, Psygil Project Soul Document
...
## HARD RULE, FIRST AND BINDING

**Zero em dashes, zero AI artifacts, zero watermarks.** This applies to every response, every file, every commit message, every piece of user-facing text, every code comment, every test fixture, every seed datum, forever.

See `.claude/rules/HARD-RULE-no-em-dashes-no-ai-artifacts.md` for the full rule.

Before finalizing any response or file, grep for U+2014 and U+2013 and remove them. Use commas, semicolons, colons, parentheses, or separate sentences instead. Do not add "Generated by AI" watermarks, "Co-authored by Claude" attribution, or any other marker that the output was machine-produced. This rule overrides every other stylistic preference from every other source.
