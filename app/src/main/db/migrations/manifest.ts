/**
 * Migration manifest for the versioned forward-migration runner.
 *
 * BUNDLING CHOICE: approach (b) from the C.5 spec.
 * electron-vite bundles the main process into a single file under out/main/.
 * Reading .sql files from disk at runtime would fail in the packaged build
 * because the migrations/ directory is not included in the asar bundle.
 * Instead, each migration's SQL is imported here as a string literal so it
 * travels inside the bundle.
 *
 * HOW TO ADD A MIGRATION:
 *   1. Create a new .sql file named NNN-description.sql in this directory.
 *   2. Add an entry to the MIGRATIONS array below with the matching version
 *      number (the NNN prefix), a short description, and the sql string.
 *   3. The runner in migrate.ts picks up the new entry automatically on
 *      the next app launch and applies it inside a transaction.
 */

/** One forward migration. version must match the NNN filename prefix. */
...
// Inline SQL for 001-initial.sql
const SQL_001 = `-- =============================================================================
-- Migration 001: Initial schema
-- All CREATE statements use IF NOT EXISTS so this is safe to run against a
-- database that was already populated before versioning landed.
-- Dependency order: practice_config -> users -> catalog/library tables ->
--   cases -> child tables -> views/FTS/triggers
-- =============================================================================
...
-- =============================================================================
-- Agent Results (shared by all 4 AI agents)
-- =============================================================================
